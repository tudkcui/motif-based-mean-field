...
if __name__ == '__main__':
    ...
    rhos = [0.2] * 3  # random fraction initially infected
    tmax = 10
    ...
    for i, N, tau, gamma, rho, P_D in zip(range(len(Ns)), Ns, taus, gammas, rhos, P_Ds):
        ...
        log = {}
        try:
            with open(f'./results/3b_logs_{i}.pkl', 'rb') as f:
                log = pickle.load(f)
                print("loaded " + f'./results/3b_logs_{i}.pkl')
        ...
        with open(f'./results/3b_logs_{i}.pkl', 'wb') as f:
            pickle.dump(log, f, 4)
    ...
    plt.tight_layout()
    plt.savefig('./figures/Fig3b_SIS_graph.pdf', bbox_inches='tight', transparent=True, pad_inches=0)
    plt.show()
