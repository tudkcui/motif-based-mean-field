...
from experiments.common import plot_AME, plot_HPA, plot_HMF, plot_Gillespie, \
    ...
    compute_degree_triangle_distribution, plot_MMF, generate_motifs_squares, compute_degree_square_distribution
...
if __name__ == '__main__':
    ...
    motifs = generate_motifs_squares()
    ...
    for i, N, tau, gamma, rho, P_D in zip(range(len(Ns)), Ns, taus, gammas, rhos, P_Ds):
        ...
        log = {}
        try:
            with open(f'./results/6_logs_{i}.pkl', 'rb') as f:
                log = pickle.load(f)
                print("loaded " + f'./results/6_logs_{i}.pkl')
        except FileNotFoundError:
            pass

        F = lambda s, t, p, q, r: tau * (p + q + 2 * r)
        R = lambda s, t, p, q, r: gamma
        Pst, Pk = compute_degree_square_distribution(P_D)
        ...
        with open(f'./results/6_logs_{i}.pkl', 'wb') as f:
            pickle.dump(log, f, 4)
    ...
    plt.tight_layout()
    plt.savefig('./figures/Fig6_SIS_square.pdf', bbox_inches='tight', transparent=True, pad_inches=0)
    plt.show()
