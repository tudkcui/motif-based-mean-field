...
if __name__ == '__main__':
    ...
    for i, N, T, rho, P_D in zip(range(len(Ns)), Ns, Ts, rhos, P_Ds):
        ...
        log = {}
        try:
            with open(f'./results/4a_logs_{i}.pkl', 'rb') as f:
                log = pickle.load(f)
                print("loaded " + f'./results/4a_logs_{i}.pkl')
        ...
        with open(f'./results/4a_logs_{i}.pkl', 'wb') as f:
            pickle.dump(log, f, 4)
    ...
    plt.tight_layout()
    plt.savefig('./figures/Fig4a_IsingGlauber_degree.pdf', bbox_inches='tight', transparent=True, pad_inches=0)
    plt.show()
